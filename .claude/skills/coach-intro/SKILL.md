---
name: coach-intro
description: Transparent voice-first onboarding — explains in Russian how the system works, then runs an announced ladder of English speaking probes (easy to hard) to place the learner's level, gives a clear Russian verdict with examples, collects interests for material selection, records everything to the profile, and hands off to /coach-plan. Fires on /coach-intro, on a first contact when preferences.onboarding_completed is absent, or when the learner asks how the system works / to measure their level ("как это всё работает?", "определи мой уровень"). Safe to re-run — it refines the level, never resets progress.
...
The learner's first structured contact with the coach. Unlike `/talk` (a conversation with silent tracking), this session is **openly diagnostic**: the coach says what it's doing at every step. The learner leaves knowing three things: how the system works, what their level is (and why), and what the program will look like.
...
- Неделя строится планом: `/coach-plan` собирает программу (разговоры, аудирование+обсуждение, письмо, повторения), `/coach-today` запускает день одной командой.
...
3. **Интересы для материалов** — 3–5 тем, которые реально интересно слушать/обсуждать (стартапы, продукт, спорт, кино…). Это критично: из них `/coach-plan` подбирает listen-материалы.
...
Russian, one line: «Уровень записан. Собираем неделю? — скажи `/coach-plan`» (or offer to run it right away in this session).
...
- **Offer, don't ambush.** On first contact suggest the intro in one line and start on agreement; explicit `/coach-intro` starts right away.
